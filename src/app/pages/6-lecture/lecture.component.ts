...
export class LectureComponent6 implements OnInit{
  ...
  private simpleRect(x1: number, y1: number, x2: number, y2: number, color = "#000", lineWidth = 2) {
    this.ctx.strokeStyle  = color;
    this.ctx.lineWidth = lineWidth;
    const min = Math.min(Math.abs(x1 - x2), Math.abs(y1 - y2));
    ...
    // this.ctx.quadraticCurveTo(...this.transfromPoint(x2, y1), ...this.transfromPoint(x2, y1));
    this.ctx.lineTo(...this.transfromPoint(x2, y2));
    // this.ctx.quadraticCurveTo(...this.transfromPoint(x2, y2), ...this.transfromPoint(x2, y2));
    this.ctx.lineTo(...this.transfromPoint(x1, y2));
    // this.ctx.quadraticCurveTo(...this.transfromPoint(x1, y2), ...this.transfromPoint(x1 , y2));
    this.ctx.lineTo(...this.transfromPoint(x1, y1));
    // this.ctx.quadraticCurveTo(...this.transfromPoint(x1, y1), ...this.transfromPoint(x1, y1));
    this.ctx.closePath();
    this.ctx.stroke();
  }
  ...
  private update = (time: number) => {
    ...
    this.simpleRect(-60, -35,  60, 35);
    // this.roundRect(0, 0,  120, 70, 15); 
